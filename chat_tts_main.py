...
@app.route("/")
def home():
    # redirects to index.html in templates folder
...
def connect():  # when socket connects, send data confirming connection
    socketio.emit('message_send', {
                  'message': "Connected successfully!", 'current_user': "Temp User", 'user_number': "1"})
...
class Bot(commands.Bot):
        ...
        if message.content == bot.keypassphrase_1:
            ...
            if bot.user_pool_1[oldest_user].replace(tzinfo=pytz.utc) < activity_threshold or len(bot.user_pool_1) > bot.max_users:
                ...
                if len(bot.user_pool_1) == bot.max_users:
                    ...
                    print(
                        f"{oldest_user} was popped due to not talking for {bot.seconds_active} seconds")
        elif message.content == bot.keypassphrase_2:
            # Remove this chatter from pool if they're already there
            if message.author.name.lower() in bot.user_pool_2:
                bot.user_pool_2.pop(message.author.name.lower())
            # Add user to end of pool with new msg time
            bot.user_pool_2[message.author.name.lower()] = message.timestamp
            # Now we remove the oldest viewer if they're past the activity threshold, or if we're past the max # of users
            activity_threshold = datetime.now(
                pytz.utc) - timedelta(seconds=bot.seconds_active)  # calculate the cutoff time
            # The first user in the dict is the user who chatted longest ago
            oldest_user = list(bot.user_pool_2.keys())[0]
            ...
            # Remove this chatter from pool if they're already there
            if message.author.name.lower() in bot.user_pool_3:
                bot.user_pool_3.pop(message.author.name.lower())
            # Add user to end of pool with new msg time
            bot.user_pool_3[message.author.name.lower()] = message.timestamp
            # Now we remove the oldest viewer if they're past the activity threshold, or if we're past the max # of users
            activity_threshold = datetime.now(
                pytz.utc) - timedelta(seconds=bot.seconds_active)  # calculate the cutoff time
            # The first user in the dict is the user who chatted longest ago
            oldest_user = list(bot.user_pool_3.keys())[0]
    ...
    def randomUser(this, user_number):
        try:
            if user_number == "1":
                bot.current_user_1 = random.choice(
                    list(bot.user_pool_1.keys()))
                socketio.emit('message_send',
                              {'message': f"{bot.current_user_1} was picked!",
                               'current_user': f"{bot.current_user_1}",
                ...
                bot.current_user_2 = random.choice(
                    list(bot.user_pool_2.keys()))
                socketio.emit('message_send',
                              {'message': f"{bot.current_user_2} was picked!",
                               'current_user': f"{bot.current_user_2}",
                               'user_number': user_number})
                ...
                bot.current_user_3 = random.choice(
                    list(bot.user_pool_3.keys()))
                socketio.emit('message_send',
                              {'message': f"{bot.current_user_3} was picked!",
                               'current_user': f"{bot.current_user_3}",
                               'user_number': user_number})
