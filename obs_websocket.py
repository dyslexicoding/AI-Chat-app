...
class OBSWebsocketsManager:
    ...
    def set_source_visibility(self, scene_name, source_name, source_visible=True):
        ...
        self.ws.call(requests.SetSceneItemEnabled(
            sceneName=scene_name, sceneItemId=myItemID, sceneItemEnabled=source_visible))
    ...
    def get_source_transform(self, scene_name, source_name):
        ...
        response = self.ws.call(requests.GetSceneItemTransform(
            sceneName=scene_name, sceneItemId=myItemID))
        transform = {}
...
if __name__ == '__main__':
    ...
    # this is for test statments needs lot of work here
    ...
    obswebsockets_manager.set_source_visibility(
        'live', "Elgato Cam Link", False)
    time.sleep(3)
    obswebsockets_manager.set_source_visibility(
        'live', "Elgato Cam Link", True)
    time.sleep(3)
    ...
    obswebsockets_manager.set_filter_visibility(
        "Middle Monitor", "DS3 - Scroll", True)
    time.sleep(3)
    obswebsockets_manager.set_filter_visibility(
        "Line In", "Audio Move - Chat God", False)
    obswebsockets_manager.set_filter_visibility(
        "Middle Monitor", "DS3 - Scroll", False)
    ...
    obswebsockets_manager.set_text(
        "??? Challenge Title ???", "Here's my new text!")
    time.sleep(3)
    ...
    obswebsockets_manager.set_source_transform(
        'live', "Middle Monitor", new_transform)
    time.sleep(3)
    print("Setting the transform back. \n\n")
    obswebsockets_manager.set_source_transform(
        'live', "Middle Monitor", transform)

    response = obswebsockets_manager.get_input_settings(
        "??? Challenge Title ???")
    ...
    response = obswebsockets_manager.get_scene_items('live')
